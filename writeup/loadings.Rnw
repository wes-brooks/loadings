...
<<label=read_data, echo=FALSE>>=
    ...
    for(stream_name in streams)
    {
        ...
        #Old way of finding snowmelt-driven events and breaking rainfall-driven events into early and late subseasons:
    	#prev_class = 0
...
<<label=bubble_plots, echo=False, include=False>>=
    ...
    for(stream_name in streams)
    {
        ...
        symbols(stream[stream$snow==FALSE,]$antecedent_qbase_quantile,
                ...
                xlab="Antecedent baseflow quantile",
...
<<label=boxplots, echo=False, include=False>>=
    ...
    par(mfg=c(1,1))
    axis(side=1, cex=0.8)
    title(xlab="log sediment yield", line=2.2, cex=0.8)
    ...
    plot.window(xlim=yy, ylim=xx)
    title(xlab="log phosphorus yield (log kg/m^2)", line=2.2, cex=0.8)
    axis(side=1, cex=0.8)
...
<<label=combined_table,echo=FALSE,results=tex>>=
    library(xtable)
    print(xtable(cbind(stot_tot_percentages, ptot_tot_percentages), caption="Proportion of total loading contributed by each type of event",
                 label="tab:stot", align=c('l', 'c', 'c', 'c', 'c')),
    caption.placement="bottom", hline.after=0 )
@

<<label=total_solids_table,echo=FALSE,results=tex,eval=FALSE>>=
    library(xtable)
...
<<label=total_phosphorus_table,echo=FALSE,results=tex,eval=FALSE>>=
    library(xtable)
