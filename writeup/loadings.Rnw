...
<<read_data>>=
    ...
    for(stream_name in streams)
    {
        ...
        stream = within( stream, {
            ...
            melt_snow <- as.numeric(levels(melt_snow)[melt_snow]) * 25.4
            sweq <- as.numeric(sub("%", "e-2", sweq)) * 25.4
            ...
            theisen <- theisen * 25.4 #Convert rainfall to mm
            nws_prec <- nws_prec * 25.4
            nws_snow <- nws_snow * 25.4
            total_water <- total_water * 25.4
            melt_water <- melt_water * 25.4
...
<<function_definitions>>=
    ...
    stepwise_BIC <- function(target, data, sites) {
        predictors_raw = c("num_events", "theisen", "p5max", "p10max", "p15max", "p30max", "p60max",
                       ...
                       "nws_prec", "cos_julian", "sin_julian",
...
\begin{figure}[h]
    \begin{center}
<<show_boxplots, fig=TRUE, fig.width=7.5, fig.height=9, include=TRUE>>=
<<boxplots>>
@
...
\begin{figure}[h]
    \begin{center}
<<show-cumulative-sediment-boxplots, fig=TRUE, fig.width=7.5, fig.height=9, include=TRUE>>=
<<cumulative-sediment-boxplots>>
@
...
\begin{figure}[h]
    \begin{center}
<<show-cumulative-phosphorus-boxplots, fig=TRUE, fig.width=7.5, fig.height=9, include=TRUE>>=
<<cumulative-phosphorus-boxplots>>
@
...
<<cumulative_loading_by_rainfall>>=
    ...
    {
        num_rows = ceiling(length(streams)/2)
        layout(matrix(1:(2*num_rows), num_rows, 2))#, widths=rep(lcm(4),2), heights=lcm(c(2,2,2,2)))
        par(mar=c(1,0,0,0), oma=c(4.5,4.5,0,1))
        
        line_type=1
        color=1
        yy=c(0,1)
        xx = c(0,50*ceiling(max(aggregate[aggregate$snow==FALSE & !is.na(aggregate$ptot_tot) & !is.na(aggregate$stot_tot),'theisen'], na.rm=T)/50))
        ...
        for(stream in streams)
        {
            ...
            if (panel==1) {legend(x='topleft', bty='n', legend=c("sediment", "phosphorus"), lty=1:2, cex=cex.legend, lwd=lwd)}
            mtext(stream_names[[stream]], side=3, line=-5, cex=cex.title, at=0.15*xx[2] )
            
            #Set the axes
            if (panel %in% 1:3) {par(new=F, ann=T, xaxt='n', yaxt='s')
            } else if (panel==4) {par(new=F, ann=T, xaxt='s', yaxt='s')
            } else if (panel==8) {par(new=F, ann=T, xaxt='s', yaxt='n')
            } else {par(new=F, ann=T, xaxt='n', yaxt='n')}
            
            #
            axis(side=1, at=seq(xx[1], xx[2], by=50), cex.axis=cex.axis, las=las, mgp=c(3,line.xaxis,0))
            axis(side=2, at=c(0, 0.5, 1), cex.axis=cex.axis, las=las)
            
            panel = panel + 1
        }
        
        mtext("Rainfall depth (mm)", side=1, line=2.5, cex=cex.lab, at=NA, outer=TRUE)
        mtext("Cumulative total loading proportion", side=2, line=2.5, cex=cex.lab, at=NA, outer=TRUE)
    }
@
...
<<figure2, fig=FALSE, include=FALSE, echo=FALSE>>=
    pdf("figure2.pdf", width=6, height=6)
    cumulative(lwd=2, las=1, line.xaxis=1.2, cex.title=1, cex.lab=1, cex.legend=1, cex.axis=1)
    dev.off()
@
...
<<figure5, include=FALSE, echo=FALSE>>=
