...
Within the category of events that are driven by rainfall, we investigate making a further split (on May 15 each year) between early post-snow and late post-snow events. Erosion may be more common early in the spring, before most of the summer's vegetation appears, which would alter the relationship between our inputs and outputs.\\

The details of that investigation are not yet included here, but the gist is that the improvement in model fit when the early and late rainfall events are separated is statistically significant but so small as to be unimportant to the ultimate goal of characterizing the way that the sediment and phosphorus get into the streams. The most important difference between the models is that the coefficient on theisen rainfall is about 20\% smaller in the late post-snow season than in the early post-snow season (of course, we don't discuss the models that use theisen rainfall and antecedent baseflow until the Modeling section). At most streams, this allowed the model to explain 1\%-2\% more of the variation in the output.\\
...
<<echo=False>>=
prp_major = proportions(streams, "stot_tot_major", col.names=c("snowmelt-driven", "early post-snow", "late post-snow"))
...
\begin{table}[h]
    ...
    \hline
    Eagle & \Sexpr{round(es$R2[1], 3)} & \Sexpr{names(es$ranked)[1]}\\
    ...
    Joos & & \\
    & \Sexpr{round(js$R2[1], 3)} & \Sexpr{names(js$ranked)[1]}\\
    ...
    Otter & & \\
    & \Sexpr{round(os$R2[1], 3)} & \Sexpr{names(os$ranked)[1]}\\
    ...
    Brewery & & \\
    & \Sexpr{round(bs$R2[1], 3)} & \Sexpr{names(bs$ranked)[1]}\\
    ...
    \hline
    Eagle & \Sexpr{round(ep$R2[1], 3)} & \Sexpr{names(ep$ranked)[1]}\\
    ...
    Joos & & \\
    & \Sexpr{round(jp$R2[1], 3)} & \Sexpr{names(jp$ranked)[1]}\\
    ...
    Otter & & \\
    & \Sexpr{round(op$R2[1], 3)} & \Sexpr{names(op$ranked)[1]}\\
    ...
    Brewery & & \\
    & \Sexpr{round(bp$R2[1], 3)} & \Sexpr{names(bp$ranked)[1]}\\
    & \Sexpr{round(bp$R2[2], 3)} & \Sexpr{names(bp$ranked)[1]} + \Sexpr{names(bp$ranked)[2]}\\
    & \Sexpr{round(bp$R2[3], 3)} & \Sexpr{names(bp$ranked)[1]} + \Sexpr{names(bp$ranked)[2]} + \Sexpr{names(bp$ranked)[3]}\\
