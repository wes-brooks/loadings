...
stepwise_BIC <- function(target, data) {
    ...
    fm2 <- lm(target~., data=d)
    return( step( fm2, direction='both', k=log(dim(d)[1]) ) )
    }

parse_args <- function(model) {
    s = unlist(strsplit(as.character( model$call )[2], "\\s"))
    indx = seq(from=3, to=length(s), by=2)
    return( s[indx] )
    }
@
...
<<label=linear_loading_model, echo=False>>=
#Solids:
summary(lm(log_stot_tot~antecedent_qbase+theisen, data=eagle_nosnow))
summary(lm(log_stot_tot~antecedent_qbase+theisen, data=joosvalley_nosnow))
summary(lm(log_stot_tot~antecedent_qbase+theisen, data=otter_nosnow))
summary(lm(log_stot_tot~antecedent_qbase+theisen, data=brewery_nosnow))
#Phosphorus:
summary(lm(log_ptot_tot~antecedent_qbase+theisen, data=eagle_nosnow))
summary(lm(log_ptot_tot~antecedent_qbase+theisen, data=joosvalley_nosnow))
summary(lm(log_ptot_tot~antecedent_qbase+theisen, data=otter_nosnow))
summary(lm(log_ptot_tot~antecedent_qbase+theisen, data=brewery_nosnow))
@
...
#Eagle creek
boxplot(log_stot_tot~event_class, data=eagle, bty='n', ylim=yy, cex=0.6, pch=4, boxwex=0.4, frame.plot=F, ylab="log(sediment load)", names=event_classes, xlim=c(1,11))
...
for(stream in streams) {
    ...
    y = cumsum(non_snow_sorted$stot_tot)/sum(stream_data$stot_tot)
...
for(stream_name in streams){
    ...
    proportion = sum(stream[stream$major==1,'stot_tot'])/sum(stream[,'stot_tot'])
...
The top 10\% of events contributed \Sexpr{round(100*q_90$eagle,1)}\% of the sediment loading at Eagle Creek, \Sexpr{round(100*q_90$otter,1)}\% of the sediment loading at Otter Creek, and \Sexpr{round(100*q_90$joosvalley,1)}\% of the sediment loading at Joos Valley Creek.\\
...
\begin{table}[h]
    ...
    \begin{tabular}{llccccc}
        ...
        \Sexpr{round(100*sum(aggregate[aggregate$event_class==1,'stot_tot'])/sum(aggregate$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(aggregate, target="stot_tot", strata="stream")==1 & aggregate$event_class==1))/sum(major(aggregate, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(aggregate[aggregate$event_class==1,'ptot_tot'])/sum(aggregate$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(aggregate, target="ptot_tot", strata="stream")==1 & aggregate$event_class==1))/sum(major(aggregate, target="ptot_tot", strata="stream")), 0)}\% \\
        & Early post-snow & 
        \Sexpr{round(100*sum(aggregate$event_class==2)/sum(aggregate$event), 0)}\% &
        \Sexpr{round(100*sum(aggregate[aggregate$event_class==2,'stot_tot'])/sum(aggregate$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(aggregate, target="stot_tot", strata="stream")==1 & aggregate$event_class==2))/sum(major(aggregate, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(aggregate[aggregate$event_class==2,'ptot_tot'])/sum(aggregate$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(aggregate, target="ptot_tot", strata="stream")==1 & aggregate$event_class==2))/sum(major(aggregate, target="ptot_tot", strata="stream")), 0)}\% \\
        & Late post-snow & 
        \Sexpr{round(100*sum(aggregate$event_class==3)/sum(aggregate$event), 0)}\% &
        \Sexpr{round(100*sum(aggregate[aggregate$event_class==3,'stot_tot'])/sum(aggregate$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(aggregate, target="stot_tot", strata="stream")==1 & aggregate$event_class==3))/sum(major(aggregate, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(aggregate[aggregate$event_class==3,'ptot_tot'])/sum(aggregate$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(aggregate, target="ptot_tot", strata="stream")==1 & aggregate$event_class==3))/sum(major(aggregate, target="ptot_tot", strata="stream")), 0)}\% \\
        
        \hline 
        \multirow{3}{*}{Eagle} & Snowmelt & 
        \Sexpr{round(100*sum(eagle$event_class==1)/sum(eagle$event), 0)}\% &
        \Sexpr{round(100*sum(eagle[eagle$event_class==1,'stot_tot'])/sum(eagle$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(eagle, target="stot_tot", strata="stream")==1 & eagle$event_class==1))/sum(major(eagle, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(eagle[eagle$event_class==1,'ptot_tot'])/sum(eagle$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(eagle, target="ptot_tot", strata="stream")==1 & eagle$event_class==1))/sum(major(eagle, target="ptot_tot", strata="stream")), 0)}\% \\
        & Early post-snow & 
        \Sexpr{round(100*sum(eagle$event_class==2)/sum(eagle$event), 0)}\% &
        \Sexpr{round(100*sum(eagle[eagle$event_class==2,'stot_tot'])/sum(eagle$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(eagle, target="stot_tot", strata="stream")==1 & eagle$event_class==2))/sum(major(eagle, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(eagle[eagle$event_class==2,'ptot_tot'])/sum(eagle$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(eagle, target="ptot_tot", strata="stream")==1 & eagle$event_class==2))/sum(major(eagle, target="ptot_tot", strata="stream")), 0)}\% \\
        & Late post-snow & 
        \Sexpr{round(100*sum(eagle$event_class==3)/sum(eagle$event), 0)}\% &
        \Sexpr{round(100*sum(eagle[eagle$event_class==3,'stot_tot'])/sum(eagle$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(eagle, target="stot_tot", strata="stream")==1 & eagle$event_class==3))/sum(major(eagle, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(eagle[eagle$event_class==3,'ptot_tot'])/sum(eagle$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(eagle, target="ptot_tot", strata="stream")==1 & eagle$event_class==3))/sum(major(eagle, target="ptot_tot", strata="stream")), 0)}\% \\
        
        \hline 
        \multirow{3}{*}{Joos} & Snowmelt & 
        \Sexpr{round(100*sum(joosvalley$event_class==1)/sum(joosvalley$event), 0)}\% &
        \Sexpr{round(100*sum(joosvalley[joosvalley$event_class==1,'stot_tot'])/sum(joosvalley$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(joosvalley, target="stot_tot", strata="stream")==1 & joosvalley$event_class==1))/sum(major(joosvalley, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(joosvalley[joosvalley$event_class==1,'ptot_tot'])/sum(joosvalley$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(joosvalley, target="ptot_tot", strata="stream")==1 & joosvalley$event_class==1))/sum(major(joosvalley, target="ptot_tot", strata="stream")), 0)}\% \\
        & Early post-snow & 
        \Sexpr{round(100*sum(joosvalley$event_class==2)/sum(joosvalley$event), 0)}\% &
        \Sexpr{round(100*sum(joosvalley[joosvalley$event_class==2,'stot_tot'])/sum(joosvalley$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(joosvalley, target="stot_tot", strata="stream")==1 & joosvalley$event_class==2))/sum(major(joosvalley, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(joosvalley[joosvalley$event_class==2,'ptot_tot'])/sum(joosvalley$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(joosvalley, target="ptot_tot", strata="stream")==1 & joosvalley$event_class==2))/sum(major(joosvalley, target="ptot_tot", strata="stream")), 0)}\% \\
        & Late post-snow & 
        \Sexpr{round(100*sum(joosvalley$event_class==3)/sum(joosvalley$event), 0)}\% &
        \Sexpr{round(100*sum(joosvalley[joosvalley$event_class==3,'stot_tot'])/sum(joosvalley$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(joosvalley, target="stot_tot", strata="stream")==1 & joosvalley$event_class==3))/sum(major(joosvalley, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(joosvalley[joosvalley$event_class==3,'ptot_tot'])/sum(joosvalley$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(joosvalley, target="ptot_tot", strata="stream")==1 & joosvalley$event_class==3))/sum(major(joosvalley, target="ptot_tot", strata="stream")), 0)}\% \\
        
        \hline 
        \multirow{3}{*}{Otter} & Snowmelt & 
        \Sexpr{round(100*sum(otter$event_class==1)/sum(otter$event), 0)}\% &
        \Sexpr{round(100*sum(otter[otter$event_class==1,'stot_tot'])/sum(otter$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(otter, target="stot_tot", strata="stream")==1 & otter$event_class==1))/sum(major(otter, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(otter[otter$event_class==1,'ptot_tot'])/sum(otter$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(otter, target="ptot_tot", strata="stream")==1 & otter$event_class==1))/sum(major(otter, target="ptot_tot", strata="stream")), 0)}\% \\
        & Early post-snow & 
        \Sexpr{round(100*sum(otter$event_class==2)/sum(otter$event), 0)}\% &
        \Sexpr{round(100*sum(otter[otter$event_class==2,'stot_tot'])/sum(otter$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(otter, target="stot_tot", strata="stream")==1 & otter$event_class==2))/sum(major(otter, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(otter[otter$event_class==2,'ptot_tot'])/sum(otter$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(otter, target="ptot_tot", strata="stream")==1 & otter$event_class==2))/sum(major(otter, target="ptot_tot", strata="stream")), 0)}\% \\
        & Late post-snow & 
        \Sexpr{round(100*sum(otter$event_class==3)/sum(otter$event), 0)}\% &
        \Sexpr{round(100*sum(otter[otter$event_class==3,'stot_tot'])/sum(otter$stot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(otter, target="stot_tot", strata="stream")==1 & otter$event_class==3))/sum(major(otter, target="stot_tot", strata="stream")), 0)}\% & 
        \Sexpr{round(100*sum(otter[otter$event_class==3,'ptot_tot'])/sum(otter$ptot_tot), 0)}\% & 
        \Sexpr{round(100*length(which(major(otter, target="ptot_tot", strata="stream")==1 & otter$event_class==3))/sum(major(otter, target="ptot_tot", strata="stream")), 0)}\% \\
    \end{tabular}
    \end{center}
\end{table}
