...
for(target in targets) {
    ...
    for(stream_name in streams){
        ...
        proportion = sum(stream[stream$major==1,target], na.rm=T)/sum(stream[,target], na.rm=T)
        q = c(q, proportion)
        assign(stream_name, stream) }
...
Over the course of the monitoring period, the majority of the total load (both of sediment and of phosphorus) was carried during just a few major events. Just 10\% of the events carried between \Sexpr{round( 100*min(q_90$stot_tot), 1 )}\% (at \Sexpr{names(q_90$stot_tot)[q_90$stot_tot==min(q_90$stot_tot)]}) and \Sexpr{round( 100*max(q_90$stot_tot), 1 )}\% (at \Sexpr{names(q_90$stot_tot)[q_90$stot_tot==max(q_90$stot_tot)]}) of the total sediment load and \Sexpr{round( 100*min(q_90$ptot_tot), 1 )}\% (at \Sexpr{names(q_90$ptot_tot)[q_90$ptot_tot==min(q_90$ptot_tot)]}) and \Sexpr{round( 100*max(q_90$ptot_tot), 1 )}\% (at \Sexpr{names(q_90$ptot_tot)[q_90$ptot_tot==max(q_90$ptot_tot)]}) of the total phosphorus load.\\
...
\begin{table}[h]
    ...
    Eagle & & \\
    & \Sexpr{round(es$R2[1], 3)} & \Sexpr{names(es$ranked)[1]}\\
    ...
    Eagle & & \\
    & \Sexpr{round(ep$R2[1], 3)} & \Sexpr{names(ep$ranked)[1]}\\
