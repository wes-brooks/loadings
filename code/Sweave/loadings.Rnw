...
for(stream_name in streams) {
    ...
    stream = within( stream, {
        ...
        julian <- as.POSIXlt( as.character(start_day), format="%m/%d/%Y")$yday + 1
        sin_julian <- sin(julian)
        cos_julian <- cos(julian)
        for(output in outputs) {
            assign( paste('log_', output, sep=""), log10(get(output)) ) }
...
#We will aggregate all the data into one frame:
...
ptot_proportions = props(streams, 'ptot_tot')
pstorm_proportions = props(streams, 'pstorm_tot')
stot_proportions = props(streams, 'stot_tot')
sstorm_proportions = props(streams, 'sstorm_tot')
...
\begin{figure}[h]
    ...
    \begin{center}
<<label=boxplot_stot, fig=True, echo=False, width=8, height=5>>=
<<stot_boxplots>>
@
...
\begin{figure}[h]
    ...
    \begin{center}
<<label=boxplot_ptot, fig=True, echo=False, width=8, height=5>>=
<<ptot_boxplots>>
@
...
\begin{figure}
    \begin{center}
<<label=figure1, fig=True, echo=False, width=6, height=5>>=
...
We can also see that the \Sexpr{round(100*sum(aggregate$event_class==1)/sum(aggregate$event), 0)}\% of the events were driven by snowmelt; these events contributed only \Sexpr{round(100*sum(aggregate[aggregate$event_class==1,'stot_tot'])/sum(aggregate$stot_tot), 0)}\% of the total sediment load but \Sexpr{round(100*sum(aggregate[aggregate$event_class==1,'ptot_tot'])/sum(aggregate$ptot_tot), 0)}\% of the total phosphorus load.

Meanwhile, the \Sexpr{round(100*sum(aggregate$event_class==2)/sum(aggregate$event), 0)}\% of events that were caused by early-season rainfall produced \Sexpr{round(100*sum(aggregate[aggregate$event_class==2,'stot_tot'])/sum(aggregate$stot_tot), 0)}\% of the total sediment load and \Sexpr{round(100*sum(aggregate[aggregate$event_class==2,'ptot_tot'])/sum(aggregate$ptot_tot), 0)}\% of the total phosphorus load.

Meanwhile, the \Sexpr{round(100*sum(aggregate$event_class==3)/sum(aggregate$event), 0)}\% of events that were caused by late-season rainfall produced \Sexpr{round(100*sum(aggregate[aggregate$event_class==3,'stot_tot'])/sum(aggregate$stot_tot), 0)}\% of the total sediment load and \Sexpr{round(100*sum(aggregate[aggregate$event_class==3,'ptot_tot'])/sum(aggregate$ptot_tot), 0)}\% of the total phosphorus load.


Overall, \Sexpr{round(100*length(which(major(aggregate, target="stot_tot", strata="stream")==1 & aggregate$event_class==1))/sum(major(aggregate, target="stot_tot", strata="stream")), 0)}\% of the largest events at each site were produced by snowmelt; \Sexpr{round(100*length(which(major(aggregate, target="stot_tot", strata="stream")==1 & aggregate$event_class==2))/sum(major(aggregate, target="stot_tot", strata="stream")), 0)}\% were produced by early-season rainfall, and \Sexpr{round(100*length(which(major(aggregate, target="stot_tot", strata="stream")==1 & aggregate$event_class==3))/sum(major(aggregate, target="stot_tot", strata="stream")), 0)}\% were produced by late-season rainfall.
